...
def parse_m3u_lines(lines, response_time):
    ...
    for line in lines:
        ...
        if line.startswith("#EXTINF"):
            match = re.search(r'group-title="(.*?)" tvg-logo="(.*?)"? tvg-id="(.*?)" tvg-name="(.*?)"?,(.*)', line)
            if match:
                current_category = match.group(1).strip()
                logo_url = match.group(2).strip() if match.group(2) else None
                tvg_id = match.group(3).strip()
                tvg_name = match.group(4).strip() if match.group(4) else None
                channel_name = match.group(5).strip()
            ...
            if current_category and channel_name:
                # 添加频道信息到当前类别中，同时记录响应时间、logo_url、tvg_id和tvg_name
                channels[current_category].append((channel_name, channel_url, response_time, logo_url, tvg_id, tvg_name))
    ...
    for line in lines:
        ...
        if "#genre#" in line:
            ...
            if match:
                ...
                tvg_id = None
                tvg_name = None
                channels[current_category].append((line, '', response_time, logo_url, tvg_id, tvg_name))

    return channels

def fuzzy_match(channel_name1, channel_name2):
    """
    模糊匹配频道名称。
    :param channel_name1: 第一个频道名称
    :param channel_name2: 第二个频道名称
    :return: 如果匹配返回True，否则返回False
    """
    channel_name1 = clean_channel_name(channel_name1)
    channel_name2 = clean_channel_name(channel_name2)
    return channel_name1 in channel_name2 or channel_name2 in channel_name1
    ...
    for category, channel_list in template_channels.items():
        ...
        for channel_name in channel_list:
            for online_category, online_channel_list in all_channels.items():
                for online_channel_name, online_channel_url, response_time, logo_url, tvg_id, tvg_name in online_channel_list:
                    if fuzzy_match(channel_name, online_channel_name):
                        # 匹配成功的频道信息加入结果中，同时记录响应时间、logo_url、tvg_id和tvg_name
                        matched_channels[category].setdefault(channel_name, []).append((online_channel_url, response_time, logo_url, tvg_id, tvg_name))
            ...
            if category in channels:
                for channel_name in channel_list:
                    if channel_name in channels[category]:
                        ...
                        for index, (url, response_time, logo_url, tvg_id, tvg_name) in enumerate(sorted_urls_ipv4, start=1):
                            new_url = add_url_suffix(url, index, total_urls_ipv4, "IPV4")
                            write_to_files(f_m3u_ipv4, f_txt_ipv4, category, channel_name, index, new_url, response_time, logo_url, tvg_id, tvg_name)

                        for index, (url, response_time, logo_url, tvg_id, tvg_name) in enumerate(sorted_urls_ipv6, start=1):
                            new_url = add_url_suffix(url, index, total_urls_ipv6, "IPV6")
                            write_to_files(f_m3u_ipv6, f_txt_ipv6, category, channel_name, index, new_url, response_time, logo_url, tvg_id, tvg_name)
...
def sort_and_filter_urls(urls, written_urls):
    ...
    """
    filtered_urls = [
        (url, response_time, logo_url, tvg_id, tvg_name) for url, response_time, logo_url, tvg_id, tvg_name in sorted(urls, key=lambda u: u[1])
        if url and url not in written_urls and not any(blacklist in url for blacklist in config.url_blacklist)
    ]
    written_urls.update([url for url, _, _, _, _ in filtered_urls])
...
def write_to_files(f_m3u, f_txt, category, channel_name, index, new_url, response_time, logo_url, tvg_id, tvg_name):
    """
    写入M3U和TXT文件。
    :param f_m3u: M3U文件对象
    :param f_txt: TXT文件对象
